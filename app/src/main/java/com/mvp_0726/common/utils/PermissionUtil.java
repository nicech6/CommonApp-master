...
/**
 * Created by creeperdch on 2018/1/1.
 * Hint:
 */
...
//                        .show();
    ...
    public static boolean verifyStoragePermissions(Activity activity) {
        ...
        try {
            ...
            int permission = ActivityCompat.checkSelfPermission(activity,
                    "android.permission.WRITE_EXTERNAL_STORAGE");
            if (flag = permission != PackageManager.PERMISSION_GRANTED) {
    ...
    public static boolean verifyDevicePermissions(Activity activity) {
        boolean flag = false;
        try {
            //检测是否有写的权限
            ...
            int permission = ActivityCompat.checkSelfPermission(activity,
                    "android.permission.WRITE_EXTERNAL_STORAGE");
            int permission_location = ActivityCompat.checkSelfPermission(activity,
                    "android.permission.ACCESS_FINE_LOCATION");
            if (flag = (permission_state != PackageManager.PERMISSION_GRANTED || permission != PackageManager.PERMISSION_GRANTED) || permission_location != PackageManager.PERMISSION_GRANTED)
            {
                // 没有写的权限，去申请写的权限，会弹出对话框
                ActivityCompat.requestPermissions(activity, PERMISSIONS_PHONE, CODE_READ_PHONE_STATE);
            }
            return flag;
        } catch (Exception e) {
            e.printStackTrace();
