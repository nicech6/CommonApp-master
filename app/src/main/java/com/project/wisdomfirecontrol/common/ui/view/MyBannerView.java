...
import com.project.wisdomfirecontrol.common.util.GlideUtils;
...
public class MyBannerView extends RelativeLayout implements View.OnTouchListener, ViewPager.OnPageChangeListener {
    ...
    private void initBannerViews(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        targetVp.addOnPageChangeListener(this);
        //添加到View中
        addView(targetVp);
    }
    ...
    public void initShowImageForLocal(Activity activity, int[] img_urls) {
        ...
        for (int i = 0; i < img_urls.length; i++) {
            ...
            //初始化指示器默认为第一张高亮
            if (i == 0) {
                iv2.setBackgroundResource(R.drawable.shape_label_red);
            } else {
                iv2.setBackgroundResource(R.drawable.shape_label_white_banner);
            }
    ...
    private void bannerPointLight(int currentPoint) {
        for (int i = 0; i < indicationList.size(); i++) {
            if (currentPoint == i) {
                indicationList.get(i).setBackgroundResource(R.drawable.shape_label_red);
            } else {
                indicationList.get(i).setBackgroundResource(R.drawable.shape_label_white_banner);
            }
        }
    }
